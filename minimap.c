        ...
        for (int x = 0; x < minimap->visible_width; ++x)
        {
            ...
            if (img->worldMap[minimap->visible_y + y][minimap->visible_x + x] == 1)
            {
                for (int i = 0; i < 180 / minimap->visible_width; ++i)
                {
                    ...
                    {
                        original_x = (i * img->textures[4].width) / (180 / minimap->visible_width);
                        original_y = (j * img->textures[4].height) / (180 / minimap->visible_height);
                        if (!img->textures[4].addr)
                            color = 0x00000;
                        else
                            color = *(unsigned int *)(img->textures[4].addr + original_y * img->textures[4].line_length + original_x * (img->textures[4].bits_per_pixel / 8));
